...
public class Main {
    ...
    public static void main(String[] args) throws RegistrationException, AuthenticationException {
        ...
        User userFromDB = authenticationService.register(
                "test@gmail.com", "13579");
        System.out.println(authenticationService.login(
                userFromDB.getEmail(), "13579"));

        ShoppingCartService shoppingCartService = (ShoppingCartService)
                injector.getInstance(ShoppingCartService.class);
        shoppingCartService.addSession(tomorrowMovieSession, userFromDB);
        ShoppingCart currentUserCart = shoppingCartService.getByUser(userFromDB);

        OrderService orderService = (OrderService) injector.getInstance(OrderService.class);
        orderService.completeOrder(currentUserCart);
