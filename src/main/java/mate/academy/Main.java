...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        todayMovieSession.setShowTime(LocalDateTime.now().plusHours(2L));
        ...
        bob.setPassword("Bob777");
        try {
            bob = authenticationService.register(bob.getEmail(), bob.getPassword());
        } catch (RegistrationException e) {
            throw new RuntimeException("Registration failed because of error " + e.getMessage());
        }
        System.out.println("User bob after registration: " + bob);
        ...
        System.out.println("Bobs shopping cart before completing order: " + bobsShoppingCart);

        OrderService orderService =
                (OrderService) injector.getInstance(OrderService.class);
        orderService.completeOrder(bobsShoppingCart);
        System.out.println("Bobs order from DB: " + orderService.getOrdersHistory(bob));
        System.out.println("Bobs shopping cart after completing order: " + bobsShoppingCart);
    }
}
